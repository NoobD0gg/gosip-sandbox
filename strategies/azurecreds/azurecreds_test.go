...
func TestGettingAuthToken(t *testing.T) {
	...
	err := h.CheckAuth(
		&AuthCnfg{},
		cnfgPath,
		[]string{"SiteURL", "TenantID", "ClientID", "Username", "Password"},
	)
	if err != nil {
		t.Error(err)
	}
...
func TestAuthEdgeCases(t *testing.T) {
	...
	t.Run("ReadConfig/MissedConfig", func(t *testing.T) {
		...
		filePath := u.ResolveCnfgPath("./tmp/private.azurecreds.malformed.json")
	...
	t.Run("WriteConfig", func(t *testing.T) {
		folderPath := u.ResolveCnfgPath("./tmp")
		filePath := u.ResolveCnfgPath("./tmp/private.azurecreds.json")
		cnfg := &AuthCnfg{
			SiteURL: "test",
		}
		_ = os.MkdirAll(folderPath, os.ModePerm)
